...
class StatelessNoisyMonitor(Monitor):
    """
    ...
    def __init__(self, env, monitor_cost=0.2, noise_scale = 0.1, **kwargs):
        gymnasium.Wrapper.__init__(self, env)
        self.action_space = spaces.Dict({
            "env": env.action_space,
    ...
    def _monitor_step(self, action, env_reward):
        if action["mon"] == 1:
            m = Normal(torch.tensor(0.0), torch.tensor(self.noise_scale))      
            proxy_reward = env_reward + m.sample().detach().numpy()
